...
const InvoiceListTable = () => {
  ...
  const { data: quotationTabs } = useSuspenseQuery(QUOTATION_TABS, { variables: { user_id: userIds.users_by_pk.id } })

  // States
  const [rowSelection, setRowSelection] = useState({})
  const [data, setData] = useState(...[quotationTabs.quotations])
    ...
    () => [
      columnHelper.accessor('id', {
        ...
        cell: ({ row }) => (
          <Typography
            component={Link}
            href={getLocalizedUrl(`/quotations/preview/${row.original.id}`, locale)}
            color='primary'
      ...
      columnHelper.accessor('total_amount', {
        header: 'Total',
        cell: ({ row }) => (
          <Typography>{`${row.original.total_amount ? row.original.total_amount.toLocaleString() : '-'}`}</Typography>
        )
      }),
